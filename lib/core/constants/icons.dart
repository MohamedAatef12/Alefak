import 'package:alefk/core/themes/app_colors.dart';
import 'package:flutter/material.dart';
import 'package:iconly/iconly.dart';

class IconlyBrokenIcons {
  // Example Icons
  static const Icon home = Icon(IconlyBroken.home);
  static const Icon profile = Icon(IconlyBroken.profile);
  static Icon send =
      Icon(IconlyBroken.send, color: AppColors.current.green, size: 30);
  static const Icon danger = Icon(IconlyBroken.danger);
  static const Icon swap = Icon(IconlyBroken.swap);
  static const Icon volumeDown = Icon(IconlyBroken.volume_down);
  static const Icon volumeUp = Icon(IconlyBroken.volume_up);
  static const Icon volumeOff = Icon(IconlyBroken.volume_off);
  static const Icon user = Icon(IconlyBroken.user_2);
  static Icon search =
      Icon(IconlyBroken.search, size: 30, color: AppColors.current.green);
  static Icon heart =
      Icon(IconlyBroken.heart, color: AppColors.current.red, size: 30);
  static Icon notification = Icon(IconlyBroken.notification,
      size: 30, color: AppColors.current.yellow);
  static const Icon settings = Icon(IconlyBroken.setting);
  static Icon chat =
      Icon(IconlyBroken.chat, color: AppColors.current.blue, size: 30);
  static const Icon camera = Icon(IconlyBroken.camera);
  static const Icon location = Icon(IconlyBroken.location);
  static const Icon bookmark = Icon(IconlyBroken.bookmark);
  static const Icon trash = Icon(IconlyBroken.delete);
  static Icon plus = Icon(
    IconlyBroken.plus,
    color: AppColors.current.gray,
  );
  static const Icon edit = Icon(IconlyBroken.edit);
  static const Icon filter = Icon(IconlyBroken.filter);
  static const Icon info = Icon(IconlyBroken.info_circle);
  static const Icon arrowRight = Icon(IconlyBroken.arrow_right);
  static const Icon arrowLeft = Icon(IconlyBroken.arrow_left);
  static const Icon arrowUp = Icon(IconlyBroken.arrow_up);
  static const Icon arrowDown = Icon(IconlyBroken.arrow_down);
  static const Icon check = Icon(IconlyBroken.tick_square);
  static const Icon close = Icon(IconlyBroken.close_square);
  static const Icon star = Icon(IconlyBroken.star);
  static const Icon logout = Icon(IconlyBroken.logout);
  static Icon upload =
      Icon(IconlyBroken.upload, color: AppColors.current.blue, size: 40);
  static const Icon download = Icon(IconlyBroken.download);
  static const Icon message = Icon(IconlyBroken.message);
  static const Icon calendar = Icon(IconlyBroken.calendar);
  static const Icon wallet = Icon(IconlyBroken.wallet);
  static const Icon call = Icon(IconlyBroken.call);
  static const Icon show = Icon(IconlyBroken.show);
  static const Icon hide = Icon(IconlyBroken.hide);
  static const Icon video = Icon(IconlyBroken.video);
  static const Icon shoppingBag = Icon(
    IconlyBroken.bag,
  );
  static Icon shoppingCart = Icon(
    IconlyBroken.buy,
    size: 30,
    color: AppColors.current.red,
  );
  static const Icon lock = Icon(IconlyBroken.lock);
  static const Icon unlock = Icon(IconlyBroken.unlock);
  static const Icon arrowDown_2 = Icon(IconlyBroken.arrow_down_2);
  static const Icon arrowRight_2 = Icon(IconlyBroken.arrow_right_2);
  static const Icon arrowLeft_2 = Icon(
    IconlyBroken.arrow_left_2,
    size: 30,
  );
  static const Icon arrowUp_2 = Icon(IconlyBroken.arrow_up_2);
  static const Icon delete = Icon(IconlyBroken.delete);
  static const Icon callMissed = Icon(IconlyBroken.call_missed);
  static const Icon bag_2 = Icon(IconlyBroken.bag_2);
  static const Icon activity = Icon(IconlyBroken.activity);
  static const Icon filter_2 = Icon(IconlyBroken.filter_2);
  static const Icon login = Icon(IconlyBroken.login);
  static const Icon work = Icon(IconlyBroken.work);

// Add more icons here based on your requirements
}
